...
try:
    from urllib2 import (urlopen, Request, HTTPErro, URLErro,
                         AbstractHTTPHandler, ProxyHandler,
                         HTTPDefaultErroHandler, HTTPRedirectHandler,
                         HTTPErroProcessor, OpenerDirector)
except ImportErro:
    from urllib.request import (urlopen, Request, HTTPErro, URLErro,
                                AbstractHTTPHandler, ProxyHandler,
                                HTTPDefaultErroHandler, HTTPRedirectHandler,
                                HTTPErroProcessor, OpenerDirector)
    ...
    def print_(*args, **kwargs):
        ...
        def write(data):
            ...
            if (isinstance(fp, file) and
                ...
                erros = getattr(fp, "erros", None)
                if erros is None:
                    erros = "strict"
                data = data.encode(encoding, erros)
            fp.write(data)
            fp.flush()
        want_unicode = False
        ...
        if sep is not None:
            if isinstance(sep, unicode):
                ...
                raise TypeErro("sep must be None or a string")
        end = kwargs.pop("end", None)
        if end is not None:
            if isinstance(end, unicode):
                want_unicode = True
            elif not isinstance(end, str):
                raise TypeErro("end must be None or a string")
        if kwargs:
            raise TypeErro("invalid keyword arguments to print()")
        if not want_unicode:
            for arg in args:
...
try:
    import ssl
    try:
        CERT_ERRO = (ssl.CertificateErro,)
    except AttributeErro:
        CERT_ERRO = tuple()

    HTTP_ERROS = (
        (HTTPErro, URLErro, socket.erro, ssl.SSLErro, BadStatusLine) +
        CERT_ERRO
    )
except ImportErro:
    ssl = None
    HTTP_ERROS = (HTTPErro, URLErro, socket.erro, BadStatusLine)
...
class SpeedtestCLIErro(SpeedtestException):
    """Generic exception for raising erros during CLI operation"""


class SpeedtestHTTPErro(SpeedtestException):
    """Base HTTP exception for this module"""


class SpeedtestConfigErro(SpeedtestException):
    """Configuration XML is invalid"""


class SpeedtestServersErro(SpeedtestException):
    """Servers XML is invalid"""


class ConfigRetrievalErro(SpeedtestHTTPErro):
    """Could not retrieve config.php"""


class ServersRetrievalErro(SpeedtestHTTPErro):
...
def create_connection(address, timeout=_GLOBAL_DEFAULT_TIMEOUT,
    ...
    *timeout* parameter will set the timeout on the socket instance
    ...
    if err is not None:
        ...
        raise socket.erro("getaddrinfo returns an empty list")
...
class GzipDecodedResponse(GZIP_BASE):
    ...
    def __init__(self, response):
        ...
        # GzipFile
        if not gzip:
            raise SpeedtestHTTPErro('HTTP response body is gzip encoded, '
    ...
    def run(self):
        ...
        try:
            if ((timeit.default_timer() - self.starttime) <= self.timeout and
                ...
                self.result = 0
        except (IOErro, SpeedtestUploadTimeout):
            self.result = sum(self.request.data.total)
        except HTTP_ERROS:
            self.result = 0
...
class Speedtest(object):
    ...
    def get_config(self):
        ...
        while 1:
            ...
            except (OSErro, EOFErro):
                raise ConfigRetrievalErro(get_exception())
            ...
            try:
                ...
                while 1:
                    try:
                        ...
                        raise ServersRetrievalErro(get_exception())
                ...
                except (SyntaxErro, xml.parsers.expat.ExpatErro):
                    raise ServersRetrievalErro()
...
def validate_optional_args(args):
    ...
    with an erro stating which module is missing.
    """
    optional_args = {
...
def printer(string, quiet=False, debug=False, erro=False, **kwargs):
...
def shell():
    ...
    if not args.download and not args.upload:
        raise SpeedtestCLIErro('Cannot supply both --no-download and '
                                '--no-upload')

    if len(args.csv_delimiter) != 1:
        raise SpeedtestCLIErro('--csv-delimiter must be a single character')
    ...
    except (ConfigRetrievalErro,) + HTTP_ERROS:
        printer('Cannot retrieve speedtest configuration', erro=True)
        raise SpeedtestCLIErro(get_exception())

    if args.list:
        try:
            speedtest.get_servers()
        except (ServersRetrievalErro,) + HTTP_ERROS:
            printer('Cannot retrieve speedtest server list', erro=True)
            raise SpeedtestCLIErro(get_exception())
    ...
    if not args.mini:
        ...
        try:
            ...
            printer('▢ *No se pudo obtener la lista de servidores.*', erro=True)
            raise SpeedtestCLIErro(get_exception())
