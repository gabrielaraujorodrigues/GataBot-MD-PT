...
import syntaxerro from 'syntax-error'
...
'U2Vzc2lvbiBlcnJvcg==', // "Session erro"
'RXJyb3I6IEJhZCBNQUM=', // "Erro: Bad MAC"
...
;['log', 'warn', 'erro'].forEach((methodName) => redefineConsoleMethod(methodName, filterStrings))
...
console.log(await tr('[⚠] No se encontró el arquivo creds.json para respaldar.'))
return
}
...
const code = lastDisconnect?.erro?.output?.statusCode || lastDisconnect?.erro?.output?.payload?.statusCode
if (code && code !== DisconnectReason.loggedOut && conn?.ws.socket == null) {
await global.reloadHandler(true).catch(console.erro)
...
let reason = new Boom(lastDisconnect?.erro)?.output?.statusCode
if (connection === 'close') {
...
console.log(chalk.bold.cyanBright(lenguajeGB['smsConexionreinício']()))
await global.reloadHandler(true).catch(console.erro)
...
process.on('uncaughtException', console.erro)
...
if (existsSync(dir)) conn.logger.info(` SE ACTULIZADO - '${filename}' CON SUCESSO`)
else {
conn.logger.warn(`SE ELIMINO UN ARQUIVO : '${filename}'`)
return delete global.plugins[filename]
}
} else conn.logger.info(`SE DETECTO UN NOVO PLUGINS : '${filename}'`)
const err = syntaxerro(readFileSync(dir), filename, {
sourceType: 'module',
allowAwaitOutsideFunction: true
})
if (err) conn.logger.erro(`SE DETECTO UN ERRO DE SINTAXIS | SYNTAX ERRO WHILE LOADING '${filename}'\n${format(err)}`)
else {
try {
const module = await import(`${global.__filename(dir)}?update=${Date.now()}`)
global.plugins[filename] = module.default || module
} catch (e) {
conn.logger.erro(`HAY UN ERRO REQUIERE EL PLUGINS '${filename}\n${format(e)}'`)
} finally {
...
spawn('ffmpeg'),
spawn('ffprobe'),
spawn('ffmpeg', ['-hide_banner', '-loglevel', 'erro', '-filter_complex', 'color', '-frames:v', '1', '-f', 'webp', '-']),
spawn('convert'),
spawn('magick'),
spawn('gm'),
spawn('find', ['--version'])
