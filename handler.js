...
this.pushMessage(chatUpdate.messages).catch(console.erro)
}
...
if (!('pasatempo' in user)) user.pasatempo = 0
if (!('tempo' in user)) user.tempo = 0
...
if (!user.mensagem) user.mensagem = 0
...
const isRProprietário = [...global.owner.map((number) => number)].map((v) => v.replace(/[^0-9]/g, '') + detectwhat).includes(m.sender)
const isProprietário = isRProprietário || m.fromMe
const isMods = isProprietário || global.mods.map((v) => v.replace(/[^0-9]/g, '') + detectwhat).includes(m.sender)
//const isPrems = isRProprietário || global.prems.map(v => v.replace(/[^0-9]/g, '') + detectwhat).includes(m.sender)
const isPrems = isRProprietário || global.db.data.users[m.sender].premiumTime > 0
...
if (!isRProprietário && opts['self']) return
...
let chat = global.db.data.chats[m.chat]
let user = global.db.data.users[m.sender]
if (!['owner-unbanchat.js'].includes(name) && chat && chat.isBanned && !isRProprietário) return
if (
...
return
if (m.text && user.banned && !isRProprietário) {
if (user.antispam > 2) return
m.reply(
`🚫 *ESTÁ BANIDO(A), NO PUEDE USAR LOS COMANDOS*\n📑 *MOTIVO: ${user.messageSpam === 0 ? 'NO ESPECIFICADO' : user.messageSpam}*\n⚠️ \`\`\`SI ESTE BOT ES CUENTA OFICIAL Y TIENE EVIDENCIA QUE RESPALDE QUE ESTE MENSAGEM ES UN ERRO, PUEDE EXPONER SU CASO EN:\`\`\`👉 *${ig}*\n👉 ${asistencia}`
)
user.antispam++
return
}
if (user.antispam2 && isRProprietário) return
...
if (adminMode && !isProprietário && !isRProprietário && m.isGroup && !isAdmin && gata) continue

if (plugin.rowner && plugin.owner && !(isRProprietário || isProprietário)) {
fail('owner', m, this)
continue
}
if (plugin.rowner && !isRProprietário) {
fail('rowner', m, this)
continue
}
if (plugin.owner && !isProprietário) {
fail('owner', m, this)
continue
}
...
console.erro(e)
if (e) {
let text = format(e) || 'Erro desconocido'
...
if (!isNumber(stat.success)) stat.success = m.erro != null ? 0 : 1
if (!isNumber(stat.last)) stat.last = now
if (!isNumber(stat.lastSuccess)) stat.lastSuccess = m.erro != null ? 0 : now
} else
stat = stats[m.plugin] = {
total: 1,
success: m.erro != null ? 0 : 1,
last: now,
lastSuccess: m.erro != null ? 0 : now
}
stat.total += 1
stat.last = now
if (m.erro == null) {
...
let texto = mid.mAviso + mid.mFake2(user)
...
if (opts['self'] && !isProprietário && !isRProprietário) return
...
owner: lenguajeGB['smsProprietário'](),
