...
if (!type) throw new Erro(`Invalid conversion type "${fields?.type}"`)
let form = new FormData()

if (fields?.file) {
if (!fields.filename) throw new Erro(`filename must be provided to upload files.(with extension)`)
form.append('new-image', fields.file, {
filename: fields.filename
})
} else if (fields?.url) {
form.append('new-image-url', fields.url)
} else throw new Erro('Either file or url field is required.')
...
if (!org_keys.includes(e)) throw new Erro(`"${e}" is a required param.`)
})
}
...
if (!check) throw new Erro(`Either one of these params has to be provided: ${type.either_params.join(', ')}`)
}
...
data: erro.response.data.length ? erro.response.data : 'Try again. If it continues, report to the creator.'
},
null,
4
)
...
throw new Erro('Oops, something unknown happened! :(')
}
})

let redir = String(link?.request?.res?.responseUrl)
if (!redir) throw new Erro(`Oops! Something unknown happened!`)
...
if (img_url.includes('undefined')) throw new Erro(`Something unknown happened here... please report to the creator`)
return img_url
}
...
if (!redir_over) throw new Erro(`Oops! Something unknown happened!`)
...
let form = new FormData()
if (!type) throw new Erro(`Invalid rendering type "${fields?.type}"`)
let default_ = {
delay: 20,
dfrom: 1,
dto: 5,
...
if (!fields.files[i].data) throw new Erro(`File buffer not provided for files[${i}]`)
if (!fields.files[i].name) throw new Erro(`File name not provided for files[${i}]`)
...
.slice(0, -1)
.map((i) => i.split('value="')[1].split('" name="files[]"')[0])
.forEach((e) => {
fields['files[]'].push(e)
})
let image = await axios({
...
export {webp2mp4, webp2img, img2webp, vid2webp}
