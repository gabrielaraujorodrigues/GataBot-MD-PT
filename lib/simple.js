...
erro(...args) {
console.log(
chalk.bold.bgRgb(247, 38, 33)('ERRO '),
`[${chalk.rgb(255, 255, 255)(new Date().toUTCString())}]:`,
...
? PATH
: Buffer.alloc(0)
if (!Buffer.isBuffer(data)) throw new TypeErro('Result is not a buffer')
...
//if (fileSize >= 100) throw new Erro('File size is too big!')
let opt = {}
        ...
        for (const media of medias) {
            ...
            if (/^https?:\/\//i.test(data.url)) {
                try {
                    ...
                    console.erro("Erro al obtener el tipo MIME:", erro);
                }
            }
...
console.erro('Tipo MIME no compatible:', contentType)
}
} catch (erro) {
console.erro('Erro al obtener el tipo MIME:', erro)
}
...
console.erro('Erro al obtener el tipo de arquivo:', erro)
}
}
...
console.erro('Incompatible MIME type:', contentType)
}
} catch (erro) {
console.erro('Failed to get MIME type:', erro)
}
} else {
try {
const type = await conn.getFile(buffer)
...
console.erro('Failed to get file type:', erro)
}
}
}
...
console.erro('Incompatible MIME types:', contentType)
}
} catch (erro) {
console.erro('Failed to get MIME type:', erro)
}
...
throw new Erro('You need to create a selection and a section first')
}
...
throw new Erro('You need to create a selection first')
}
const buttonParams = JSON.parse(this._buttons[this._currentSelectionIndex].buttonParamsJson)
...
if (!path) throw new Erro('URL or buffer needed')
...
value(text = '') {
if (!text) {
//console.erro('El valor de text es inválido o falsy:', text)
return []
}
...
conn.processMessageStubType(message).catch(console.erro)
...
let chats = conn.chats[chat]
if (!chats) {
if (isGroup) await conn.insertAllGroup().catch(console.erro)
...
if (inp.length !== out.length) throw new Erro('Input and Output must have same length')
