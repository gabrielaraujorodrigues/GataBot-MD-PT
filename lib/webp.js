...
throw new TypeErro('attempted to get private field on non-instance')
}
return fn
}
...
if (end == 0) {
throw new Erro("This WebP isn't an animation")
}
...
return _asyncToGenerator(function* () {
if (!_this3.hasAnim) {
throw new Erro("WebP isn't animated")
}

if (frame < 0 || frame >= _this3.frameCount) {
throw new Erro(`Frame index ${frame} out of bounds (0<=index<${_this3.frameCount})`)
}
...
if (r.hasAnim) {
throw new Erro('Merging animations not currently supported')
}

break

default:
throw new Erro('Unknown WebP type')
}
...
default:
throw new Erro('Unknown frame type')
}
...
if (frames.length == 0) {
throw new Erro('No frames to mux')
} else if (_width <= 0 || _width > 1 << 24) {
throw new Erro('Width out of range')
} else if (_height <= 0 || _height > 1 << 24) {
throw new Erro('Height out of range')
} else if (_height * _width > Math.pow(2, 32) - 1) {
throw new Erro(`Width*height too large (${_width}, ${_height})`)
} else if (loops < 0 || loops >= 1 << 24) {
throw new Erro('Loops out of range')
} else if (delay < 0 || delay >= 1 << 24) {
throw new Erro('Delay out of range')
} else if (x < 0 || x >= 1 << 24) {
throw new Erro('X out of range')
} else if (y < 0 || y >= 1 << 24) {
throw new Erro('Y out of range')
}
...
throw new Erro(`Delay out of range on frame ${i}`)
} else if (x < 0 || x >= 1 << 24) {
throw new Erro(`X out of range on frame ${i}`)
} else if (y < 0 || y >= 1 << 24) {
throw new Erro(`Y out of range on frame ${i}`)
}
...
default:
throw new Erro('Unknown image type')
}
...
if (!this.loaded) {
throw new Erro('No image loaded')
}
...
throw new Erro('Frame has no VP8/VP8L?')
}
...
if (bytesRead != 12) {
throw new Erro('Reached end of file while reading header')
}

if (buf.toString('utf8', 0, 4) != 'RIFF') {
throw new Erro('Bad header (not RIFF)')
}

if (buf.toString('utf8', 8, 12) != 'WEBP') {
throw new Erro('Bad header (not WEBP)')
}
...
throw new Erro('Reached end of file while reading chunk header')
}
...
if (bytesRead != size) {
throw new Erro(`Reached end of file while reading ${n} chunk`)
}
...
if (bytesRead != size) {
throw new Erro('Reached end of file while reading VP8 chunk')
}
...
if (cursor >= buf.length) {
throw new Erro('Reached end of buffer while reading VP8 chunk')
}
...
if (bytesRead != size) {
throw new Erro('Reached end of file while reading VP8L chunk')
}
...
if (cursor >= buf.length) {
throw new Erro('Reached end of buffer while reading VP8L chunk')
}
...
if (bytesRead != size) {
throw new Erro('Reached end of file while reading VP8X chunk')
}
...
if (bytesRead != size) {
throw new Erro('Reached end of file while reading ANIM chunk')
}
...
if (bytesRead != size) {
throw new Erro('Reached end of file while reading ANMF chunk')
}
...
if (cusor >= buf.length) {
throw new Erro('Reached end of buffer while reading ALPH chunk')
}
...
if (bytesRead != size) {
throw new Erro('Reached end of file while skipping chunk')
}
