const userSpamData = {}
let handler = (m) => m
handler.before = async function (m, {conn, isAdmin, isBotAdmin, isProprietário, isRProprietário, isPrems}) {
...
if (isProprietário || isRProprietário || isAdmin || isPrems) return
} else {
if (isProprietário || isRProprietário || isPrems) return
}

let user = global.db.data.users[m.sender]
const sender = m.sender
const currentTime = new Date().getTime()
const timeWindow = 5000 // tempo límite
const messageLimit = 10 // cantidad de mensagems en dicho tempo

let time, time2, time3, mensagem, motive
time = 30000 // 30 seg
time2 = 60000 // 1 min
time3 = 120000 // 2 min
