...
let handler = async (m, {conn, command, usedPrefix, args, text, groupMetadata, isProprietário, isRProprietário}) => {
...
let inviteCode
if (!text) return await m.reply('*⚠️ Ingrese un link de un grupo/comunidad/canal de WhatsApp para obtener informação.*')
...
`✏️ *Nombre cambiado por:*\n${res.subjectProprietário ? `@${res.subjectProprietário?.split('@')[0]}` : 'No encontrado'} ${res.subjectTime ? `el ${formatDate(res.subjectTime)}` : '(Fecha no encontrada)'}\n\n` +
`📄 *Descrição:*\n${res.desc || 'No encontrado'}\n\n` +
`📝 *Descrição cambiado por:*\n${res.descProprietário ? `@${res.descProprietário?.split('@')[0]}` : 'No encontrado'}\n\n` +
`🗃️ *Id de la descrição:*\n${res.descId || 'No encontrado'}\n\n` +
`🖼️ *Imagen del grupo:*\n${pp ? pp : groupPicture}\n\n` +
...
`✏️ *Nombre cambiado por:*\n${subjectProprietário ? `@${subjectProprietário?.split('@')[0]}` : 'No encontrado'} ${subjectTime ? `el ${formatDate(subjectTime)}` : '(Fecha no encontrada)'}\n\n` +
`📄 *Descrição:*\n${desc || 'No encontrada'}\n\n` +
...
//if (!inviteUrl &&) return await conn.reply(m.chat, "*Verifique que sea un link de grupo o comunidad de WhatsApp.*", m)
let inviteInfo
if (inviteUrl) {
try {
inviteInfo = await conn.groupGetInviteInfo(inviteUrl)
info = await inviteGroupInfo(inviteInfo) // Para cualquier link de grupo/comunidad
console.log(info)
console.log('Método de link')
} catch (e) {
...
// Manejo de links de canales
let newsletterInfo
if (!channelUrl) return await conn.reply(m.chat, '*Verifique que sea un link de canal de WhatsApp.*', m)
if (channelUrl) {
try {
newsletterInfo = await conn.newsletterMetadata('invite', channelUrl).catch((e) => {
return null
})
if (!newsletterInfo) return await conn.reply(m.chat, '*No se encontró informação del canal.* Verifique que el link sea correto.', m)
let caption = '*Inspector de links de Canales*\n\n' + processObject(newsletterInfo, '', newsletterInfo?.preview)
...
case 'description':
case 'name':
return value || 'No hay informação disponible'
case 'state':
switch (value) {
case 'ACTIVE':
return 'Activo'
...
default:
return value !== null && value !== undefined ? value.toString() : 'No hay informação disponible'
}
}
...
.replace('Description Time', '📝 Fecha de modificación de la descrição')
.replace('Description', '📜 Descrição')
.replace('Invite', '📩 Invitación')
.replace('Handle', '👤 Apelido')
...
let handler = async (m, { conn, text }) => {
	...
	let res = await conn.query({ tag: 'iq', attrs: { type: 'get', xmlns: 'w:g2', to: '@g.us' }, content: [{ tag: 'invite', attrs: { code } }] }),
		...
		pp = await conn.profilePictureUrl(data.id, 'image').catch(console.erro)
let groupinfo = `*┏━━━━━━━━━━━━━━━┓*
*┃☂️ ⫹⫺ ID:* ${data.id}◞
*┃🧪 ⫹⫺ Nombre:* ${data.subject}
...
await await conn.sendButton(m.chat, groupinfo, `*Copiar Descrição 👇*`, pp, [['𝙑𝙤𝙡𝙫𝙚𝙧 𝙖𝙡 𝙈𝙚𝙣𝙪́ | 𝘽𝙖𝙘𝙠 𝙩𝙤 𝙈𝙚𝙣𝙪', '/menu']], `https://www.whatsapp.com/otp/copy/${data.desc}`, null, null, m)
