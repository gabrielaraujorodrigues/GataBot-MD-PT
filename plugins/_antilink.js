...
let handler = (m) => m
handler.before = async function (m, {conn, isAdmin, isBotAdmin, isProprietário, isRProprietário, participants}) {
if (!m.isGroup) return
if (isAdmin || m.fromMe || isRProprietário || isProprietário || !isBotAdmin) return
...
await conn.sendMessage(m.chat, {text: `${mid.mAviso + mid.mWhatsApp}..`}, {quoted: m})
