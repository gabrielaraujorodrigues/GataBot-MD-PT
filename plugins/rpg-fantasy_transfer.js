...
fake = {
contextInfo: {
externalAdReply: {title: 'Transfiera a otro Usuário 🧐', body: 'Algo no salió bien...', sourceUrl: accountsgb, thumbnailUrl: gataMenu}
}
}
...
fake = {
contextInfo: {
externalAdReply: {title: '❌ Parámetros incompletos', body: 'Algo no salió bien...', sourceUrl: accountsgb, thumbnailUrl: gataMenu}
}
}
return conn.reply(
m.chat,
`*Use un caracter en medio del Usuário y personaje*\n\n> *Caracteres aceptados:*\n\`(|), (,), (\\), (&), y (/)\`\n\n> *Ejemplo:*\n\`${usedPrefix + command} Usuário | Personaje\`\n\n> *Para ver tus Personajes, escriba:*\n\`${usedPrefix}fantasymy o ${usedPrefix}fymy\``,
m,
fake
)
}
...
return conn.reply(
m.chat,
`*Responda a un mensagem de @${m.quoted.sender.split('@')[0]} escribiendo el nombre o código del personaje*\n\n> *Para ver tus Personajes, escriba:*\n\`${usedPrefix}fantasymy o ${usedPrefix}fymy\``,
m,
...
return conn.reply(
m.chat,
`*Etiqueta o escriba el número del usuário y nombre o código del personaje*\n\n> *Ejemplo:*\n\`${usedPrefix + command} usuário | personaje\`\n\n> _También puede responder al mensagem del usuário escribiendo el nombre o código del personaje_\n\n> *Para ver tus Personajes, escriba:*\n\`${usedPrefix}fantasymy o ${usedPrefix}fymy\``,
m
)
}
...
return conn.reply(
m.chat,
`*O usuário @${user.split('@')[0]} no puede recibir transferencias de personajes*\n\n> *Motivo:* _Sólo puedes transferir tus personajes a usuários que hayan comprado mínimo un personaje_\n\n*@${user.split('@')[0]} Compra un personaje para recibir/enviar transferencias*\n\`${usedPrefix}fantasy o ${usedPrefix}fy\``,
m,
{mentions: [user]}
)
...
return conn.reply(
m.chat,
`*No hemos encontrado "${character}"*\n\n> *Motivo:* _Puede deberse a que no tiene ese personaje o está mal escrito el nombre o código del personaje_\n\n> *Para ver tus Personajes, escriba:*\n\`${usedPrefix}fantasymy o ${usedPrefix}fymy\``,
m
)

let id_message
if (characterIndex != -1) {
let mensagemConfirmacion = `> *Esto pasará si transfieres "${senderData.fantasy[characterIndex].name}" a @${user.split('@')[0]}*\n
...
> _Si deseas continuar con la transferencia, escriba *"Si"* respondiendo a este mensagem, de lo contrario escriba *"No"*_`
id_message = (await conn.reply(m.chat, mensagemConfirmacion, m, {mentions: [user]})).key.id
}
...
let usuárioExistente = fantasyDB.find((usuário) => Object.keys(usuário)[0] === m.sender)
handler.before = async (m) => {
let senderCharacter
if (!usuárioExistente || !usuárioExistente[m.sender]?.fantasy?.some((personaje) => personaje.id === imageInfo.code)) return
...
fake = {
contextInfo: {
externalAdReply: {title: '✋ Decidió no continuar', body: 'No se hizo transferencia', sourceUrl: accountsgb, thumbnailUrl: gataMenu}
}
}
