...
if (!m.quoted && !text) throw '*⚠️ Ingrese un texto o responda a un arquivo multimedia para subir como estado.*'
...
throw '*⚠️ No se pudo procesar el arquivo o mensagem para subir como estado.*'
}
...
let handler = async (m, { conn, text }) => {
    ...
    if (!m.quoted && !text) throw '*_Ingrese un texto o reponda a algun arquivo multimedia_*'
    ...
    m.reply('*✅ Estado subido con exito, agenda en tu lista de contactos el número del Bot y pide al proprietário del Bot que te agregue a sus contactos para que puedas ver los estados*')
    }
handler.help = ['subirestado']
handler.tags = ['General']
handler.command = /^subirestado$/i
