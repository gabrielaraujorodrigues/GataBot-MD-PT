import ws from 'ws'
const comandos =
/piedra|papel|tijera|estado|verificar|code|jadibot --code|--code|criadora|bottemporal|grupos|instalarbot|términos|bots|deletebot|removersesion|serbot|verify|register|registrar|reg|reg1|nombre|name|nombre2|name2|edad|age|edad2|age2|genero|género|gender|identidad|pasatempo|hobby|identify|finalizar|pas2|pas3|pas4|pas5|registroc|deletesesion|registror|jadibot/i

export async function before(m, {conn, isAdmin, isBotAdmin, isProprietário, isRProprietário}) {
...
//contando de mensagem
if (!global.db.data.users[m.sender]) global.db.data.users[m.sender] = {}
if (!global.db.data.users[m.sender].mensagem) global.db.data.users[m.sender].mensagem = {}
if (!global.db.data.users[m.sender].mensagem[m.chat]) global.db.data.users[m.sender].mensagem[m.chat] = 0
global.db.data.users[m.sender].mensagem[m.chat]++
...
//antipv
if (setting.antiPrivate && !isProprietário && !isRProprietário) {
if (user.warnPv) {
...
await conn.reply(m.chat, mid.mAviso + mid.smsprivado(m, cuentas), m, {mentions: [m.sender]})
user.warnPv = true
throw !0
}
}
