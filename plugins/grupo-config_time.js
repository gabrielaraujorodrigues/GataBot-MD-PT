...
let handler = async (m, {conn, isAdmin, isProprietário, args, usedPrefix, command}) => {
try {
...
let sections = Object.keys(nombre, descrição, comando).map((v, index) => ({
title: `${lenguajeGB.smsParaAdmins()}`,
rows: [{title: `${nombre[v]}`, description: `${descrição[v]}`, rowId: usedPrefix + command + ' ' + comando[v]}]
}))
...
await conn.reply(
m.chat,
`${lenguajeGB['smsAvisoMG']()} ${lenguajeGB['smsMalused']()} 
${lenguajeGB['smsGrupoTime1']()}
*${usedPrefix + command} ${lenguajeGB.lenguaje() == 'en' ? 'open' : 'abrir'} 1*
${lenguajeGB['smsGrupoTime2']()}
*${usedPrefix + command} ${lenguajeGB.lenguaje() == 'en' ? 'close' : 'cerrar'} 1*`,
...
m.reply(
`${lenguajeGB['smsAvisoRG']()} *${lenguajeGB['smsGrupoTime3']()}* *${isClose == 'announcement' ? lenguajeGB.smsGrupoTime4() : lenguajeGB.smsGrupoTime5()} ${args[1] ? `${lenguajeGB['smsGrupoTime6']()} ${clockString(timeoutset)}*` : ''}`
)
})
if (args[1]) {
setTimeout(async () => {
...
handler.command = /^(grouptime|gctime|grupotempo)$/i
