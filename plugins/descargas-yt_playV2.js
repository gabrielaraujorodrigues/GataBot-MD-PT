...
if (!axeelData || !axeelData.downloads?.url) throw new Erro()
...
console.erro('Erro al obtener el audio:', e)
...
let {data} = await res.json()
await conn.sendFile(m.chat, data.dl, 'erro.mp4', `${gt}`, m)
} catch (e1) {
try {
const axeelUrl = `https://axeel.my.id/api/download/audio?url=${yt_play[0].url}`
const axeelResponse = await fetch(axeelUrl)
const axeelData = await axeelResponse.json()
if (!axeelData || !axeelData.downloads?.url) throw new Erro()
await conn.sendFile(m.chat, axeelData.downloads.url, 'erro.mp4', `${gt}`, m)
} catch {
try {
const ryzenUrl = `https://api.ryzendesu.vip/api/downloader/ytmp3?url=${yt_play[0].url}`
const ryzenResponse = await fetch(ryzenUrl)
...
if (ryzenData.status === 'tunnel' && ryzenData.url) {
const downloadUrl = ryzenData.url
await conn.sendFile(m.chat, downloadUrl, 'erro.mp4', `${gt}`, m)
}
} catch {
try {
let d2 = await fetch(`https://exonity.tech/api/ytdlp2-faster?apikey=adminsepuh&url=${yt_play[0].url}`)
...
const audiop = await getBuffer(dp.result.media.mp3)
const fileSize = await getFileSize(dp.result.media.mp3)
await conn.sendFile(m.chat, audiop, 'erro.mp4', `${gt}`, m)
} catch (e) {
console.erro('Erro al obtener el audio:', e)
await conn.reply(m.chat, `${lenguajeGB['smsMalErro3']()}#report ${lenguajeGB['smsMensErro2']()} ${usedPrefix + command}\n\n${wm}`, m)
}
...
const aa = await conn.sendMessage(m.chat, {audio: {url: audiocore}, fileName: `erro.mp3`, mimetype: 'audio/mp4'}, {quoted: m});
if (!aa) {
throw new Erro();
}} catch {
try{const res = await fetch(`https://skizo.tech/api/y2mate?apikey=GataDios&url=${text}`);
const json = await res.json();
const aa_1 = await conn.sendMessage(m.chat, {audio: {url: json.result.audio}, fileName: `erro.mp3`, mimetype: 'audio/mp4'}, {quoted: m});
if (!aa_1) aa_1 = await conn.sendFile(m.chat, json.result.audio, 'erro.mp3', null, m, false, {mimetype: 'audio/mp4'});
}catch {
try{
let res = await yts(text)
res=res.videos[0]
let yt = await fg.yta(res.url,'128kbps')
await conn.sendMessage(m.chat, {audio: {url: yt.dl_url}, fileName: `erro.mp3`, mimetype: 'audio/mp4'}, {quoted: m});
}catch(e){ 
await conn.reply(m.chat, `${lenguajeGB['smsMalErro3']()}#report ${lenguajeGB['smsMensErro2']()} ${usedPrefix + command}\n\n${wm}`, m)
console.log(`❗❗ ${lenguajeGB['smsMensErro2']()} ${usedPrefix + command} ❗❗`)
console.log(e)
...
const aa_2 = await conn.sendMessage(m.chat, {video: {url: mediaa.result}, fileName: `erro.mp4`, caption: `${wm}`, thumbnail: mediaa.thumb, mimetype: 'video/mp4'}, {quoted: m});
if (!aa_2) {
throw new Erro();
}} catch {
try
...
await conn.sendFile(m.chat, yt0.dl_url, 'erro.mp4', `${wm}`, m);
}
catch{
const res = await fetch(`https://skizo.tech/api/y2mate?apikey=GataDios&url=${text}`);
const json = await res.json();
await conn.sendFile(m.chat, json.result.video, 'erro.mp4', `${wm}`, m);  
}
}}} catch (e) {
await conn.reply(m.chat, `${lenguajeGB['smsMalErro3']()}#report ${lenguajeGB['smsMensErro2']()} ${usedPrefix + command}\n\n${wm}`, m)
console.log(`❗❗ ${lenguajeGB['smsMensErro2']()} ${usedPrefix + command} ❗❗`)
...
conn.sendFile(m.chat, json.result.url, 'erro.mp3', null, m, false, { mimetype: 'audio/mp4' })}
if (command == 'play.2') {
...
let res = await fetch("https://violetics.pw/api/media/youtube-play?apikey=beta&query="+text) 
let json = await res.json()
conn.sendFile(m.chat, json.result.url, 'erro.mp4', `${wm}`, m)}
}catch(e){
