...
if (!api?.status || !api?.data?.download) throw new Erro('No se pudo obtener el link de descarga.')
...
await delay(2000)
await conn.sendMessage(m.chat, {image: {url: portada}, caption: `Aguarde por favor...\n\nEnviando: ${ddname}\n\n${wm}`}, {quoted: m})
...
console.log('❌ soundcloud dl erro:', e?.message || e)
await m.reply('❌ Ocurrió un erro al procesar tu solicitud.')
}
} catch (e) {
return await m.reply('❌ Ocurrió un erro al procesar tu solicitud.')
}
}
