...
return conn.reply(m.chat, `${lenguajeGB['smsAvisoAG']()}${lenguajeGB['smsSoloProprietário']()}`, fkontak, m)
...
if (!global.db.data.settings[conn.user.jid].restrict) return conn.reply(m.chat, `${lenguajeGB['smsAvisoAG']()}${lenguajeGB['smsSoloProprietário']()}`, fkontak, m) 
try {
...
for (const user of participant.filter(item => item.attrs.erro == 403)) {
