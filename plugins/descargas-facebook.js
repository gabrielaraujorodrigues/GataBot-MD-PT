...
let conteúdo = `✅ 𝙑𝙄𝘿𝙀𝙊 𝘿𝙀 𝙁𝘼𝘾𝙀𝘽𝙊𝙊𝙆\n${wm}`
await m.react('⏱️')
try {
...
const conteúdo = `Resolución: ${videoSeleccionado.resolution}`

await conn.sendFile(m.chat, downloadUrl, 'video.mp4', conteúdo, m)
await m.react('✅')
} catch {
try {
...
await conn.sendFile(m.chat, imageUrl, 'thumbnail.jpg', conteúdo, m, null, fake)
await m.react('✅')
}
} catch {
try {
...
await conn.sendFile(m.chat, videoUrl, 'video.mp4', conteúdo, m, null, fake)
await m.react('✅')
}
} catch {
try {
...
await conn.sendFile(m.chat, downloadUrl, 'erro.mp4', conteúdo, m)
} catch {
try {
const ress = await fg.fbdl(args[0])
const urll = await ress.data[0].url
await conn.sendFile(m.chat, urll, 'erro.mp4', conteúdo, m)
await m.react('✅')
} catch {
try {
...
await conn.sendFile(m.chat, url, quality || '', conteúdo, m)
} catch (e) {
console.log(e)
}
}
