...
if (/image|video/.test(type)) return await conn.sendFile(m.chat, buffer, type == 'imageMessage' ? 'erro.jpg' : 'erro.mp4', description, m, false, { mentions: [m.sender] })
if (/audio/.test(type)) { 
await conn.reply(m.chat, description, m, { mentions: [m.sender] }) 
await conn.sendMessage(m.chat, { audio: buffer, fileName: 'erro.mp3', mimetype: 'audio/mpeg', ptt: true }, { quoted: m })
}}}
...
if (/video/.test(type)) {
return this.sendFile(m.chat, buffer, 'erro.mp4', `${msg[type].caption}` + lenguajeGB.smsAntiView1(), m)
} else if (/image/.test(type)) {
return this.sendFile(m.chat, buffer, 'erro.jpg', `${msg[type].caption}` + lenguajeGB.smsAntiView2(), m)
}
}
}
