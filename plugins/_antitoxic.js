...
let handler = (m) => m
handler.before = async function (m, {conn, isAdmin, isBotAdmin, isProprietário}) {
...
if (isToxic && chat.antitoxic && !isProprietário && !isAdmin && isBotAdmin) {
if (chat.delete) return conn.reply(m.chat, mid.mAviso + mid.mAntiDelete, m)
user.warn += 1
if (!(user.warn >= 4)) {
