...
// Si el usuário no existe en la base de datos borra su contador de registro
...
// Verifica si el usuário existe en la base de datos y si tiene la estructura fantasy
usuárioExistente = fantasyDB.find((user) => Object.keys(user)[0] === userId && user[userId].fantasy)
if (usuárioExistente && user.fantasy_character === 0) {
fake = {
contextInfo: {
externalAdReply: {title: '🌟 NOVO LOGRO 🌟', body: 'Califica personajes, es gratis ❤️', sourceUrl: accountsgb, thumbnailUrl: gataMenu}
}
}
...
usuárioExistente = fantasyDB.find((user) => Object.keys(user)[0] === userId)
if (usuárioExistente) {
const fantasyArray = usuárioExistente[userId].fantasy
fake = {
contextInfo: {
externalAdReply: {title: '🌟 RECOMPENSA 🌟', body: 'Usa #fymy para ver más desafíos', sourceUrl: accountsgb, thumbnailUrl: gataMenu}
}
}
...
usuárioExistente = fantasyDB.find((user) => Object.keys(user)[0] === userId)
if (usuárioExistente) {
const flowArray = usuárioExistente[userId].flow || []
const likesCount = flowArray.filter((voto) => voto.like).length
fake = {
contextInfo: {
externalAdReply: {title: 'SIGUE DANDO 👍', body: 'Califica personajes, es gratis 👍', sourceUrl: accountsgb, thumbnailUrl: gataMenu}
}
}
...
fake = {
contextInfo: {
externalAdReply: {title: 'SIGUE DANDO ❤️', body: 'Califica personajes, es gratis ❤️', sourceUrl: accountsgb, thumbnailUrl: gataMenu}
}
}
