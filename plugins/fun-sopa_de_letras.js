...
const LADO = 16 // Si es alto o bajo, puede dar erro, deja como esta
...
sopaPalabra = PALABRA
sopaDir = DIRECCION.replace(/([A-Z])/g, ' $1')
.toLowerCase()
...
await conn.reply(
m.chat,
`*@${m.sender.split('@')[0]} EL TEMPO SE HA ACABADO!!* 😧\n\n*LA PALABRA _"${sopaPalabra}"_ SE ENCONTRABA EN LA DIRECCIÓN _${sopaDir}_ DE LA FILA _${fila}_ Y COLUMNA _${columna}_*`,
m,
{mentions: [m.sender]}
)
...
await m.reply(
`\`\`\`🎊 HAS GANADO ${diamante} ${rpgshop.emoticon('limit')}!!\`\`\`\n\n*CORRETO!! LA PALABRA _"${sopaPalabra}"_ SE ENCONTRABA EN LA DIRECCIÓN _${cambioLetra}_ DE LA FILA _${fila}_ Y COLUMNA _${columna}_*`
)
;((fila = null), (columna = null), (sopaNube = null), (sopaPalabra = null), (sopaDir = null), (userSP = null), (cambioLetra = null))
intentos = 0
return
...
intentos = 0
await m.reply(
`🫡 *AGOTASTE LOS INTENTOS!! LA PALABRA _"${sopaPalabra}"_ SE ENCONTRABA EN LA DIRECCIÓN _${cambioLetra}_ DE LA FILA _${fila}_ Y COLUMNA _${columna}_*`
)
return
} else {
intentos -= 1
await m.reply(
`😮‍💨 *INCORRETO. TE QUEDAN _${intentos}_ INTENTOS!!*${intentos === 1 ? '' : `\n*PALABRA A ENCONTRAR:* \`\`\`${sopaPalabra}\`\`\``}\n\n${intentos === 1 ? `\`\`\`💡 PISTA!!\`\`\`\n*LA PALABRA _${sopaPalabra}_ SE ENCUENTRA EN LA DIRECCIÓN _"${cambioLetra}"_*\n\n` : ''}${sopaNube}`
        )
        return
      }
    }
