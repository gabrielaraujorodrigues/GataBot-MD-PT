...
if (!global.db.data.settings[conn.user.jid].restrict) throw `${lenguajeGB['smsAvisoAG']()}${lenguajeGB['smsSoloProprietário']()}`
...
if (!global.db.data.settings[conn.user.jid].restrict) return conn.sendButton(m.chat, wm, `${lenguajeGB['smsAvisoAG']()}${lenguajeGB['smsSoloProprietário']()}`, null, [[lenguajeGB.smsEncender(), `${usedPrefix}on restringir`]], fkontak, m)
if (!m.mentionedJid[0] && !m.quoted) return m.reply(texto, m.chat, { mentions: conn.parseMention(texto)}) 
let user = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted.sender
let owr = m.chat.split`-`[0]
let remover = await conn.groupParticipantsUpdate(m.chat, [user], 'remove')
...
//let remover = await conn.groupParticipantsUpdate(m.chat, [mentioned], 'remove')
...
if (remover[0].status === "200") m.reply(done, m.chat, { mentions: conn.parseMention(done)})  
else if (remover[0].status === "406") m.reply(err1, m.chat, { mentions: conn.parseMention(err1)})   
else if (remover[0].status === "404") m.reply(err2, m.chat, { mentions: conn.parseMention(err2)})  
