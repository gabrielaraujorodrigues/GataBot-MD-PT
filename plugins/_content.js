...
isForwarded: true,
forwardedNewsletterMessageInfo: {newsletterJid: channelRD.id, serverMessageId: '', newsletterName: channelRD.name},
externalAdReply: {title: wm, body: vs, mediaType: 1, renderLargerThumbnail: true, previewType: 'PHOTO', thumbnailUrl: pp, sourceUrl: accountsgb}
}
}

const estilo = {
key: {fromMe: false, participant: '0@s.whatsapp.net', ...(m.chat ? {remoteJid: '5219992095479-1625305606@g.us'} : {})},
message: {
orderMessage: {
itemCount: -999999,
status: 1,
...
message: {orderMessage: {itemCount: 99, status: 1, surface: 1, message: wm, orderTitle: wm, sellerJid: '0@s.whatsapp.net'}}
}
let fgif = {
...
global.mensagemsEditados = async function (conn, m) {
const mensagems = [waitt, waittt, waitttt, waittttt]
key = await conn.sendMessage(m.chat, {text: wait, quoted: m})
for (let i = 0; i < mensagems.length; i++) {
await new Promise((resolve) => setTimeout(resolve, 1000))
await conn.sendMessage(m.chat, {text: mensagems[i], edit: key})
}
}
