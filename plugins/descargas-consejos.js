...
let conteúdo = list[Math.floor(Math.random() * list.length)]
let result = await translate(`${conteúdo}`, {to: lenguajeGB.lenguaje(), autoCorrect: true})
let texto = `
・☘️・》・》・》
...
//conn.sendFile(m.chat, img, 'erro.png', texto.trim(), fkontak)}
...
let conteúdo = await list[Math.floor(list.length * Math.random())]
let frase = conteúdo.motivasi
...
//conn.sendFile(m.chat, img, 'erro.png', texto.trim(), m, fkontak)}
