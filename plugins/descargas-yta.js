...
let handler = async (m, { text, conn, args, usedPrefix, command }) => {
  ...
  try {
    ...
    if (!target) throw new Erro('No se encontró URL válida de YouTube ni resultados de búsqueda.')
    ...
    console.log(`Erro en hiudyy ytmp3`)
    try {
      const target = youtubeLink || yt_play?.[0]?.url
      if (!target) throw new Erro('No se encontró URL válida de YouTube ni resultados de búsqueda.')
      ...
      console.log(`❌ Erro Sanka Vollerei: ${e2?.message || e2}`)
      await conn.reply(
        m.chat,
...
async function getFromSanka(youtubeUrl) {
  ...
  if (!res.ok) throw new Erro(`HTTP ${res.status}`)

  const json = await res.json().catch(() => null)
  if (!json?.status || !json?.result?.download) {
    throw new Erro('Respuesta inválida de Sanka Vollerei')
  }
