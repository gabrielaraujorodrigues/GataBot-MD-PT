...
if (!who) return m.reply(`${ag} *ETIQUETE AL USUÁRIO*`)
if (!(who in global.db.data.users)) return m.reply(`${fg}*EL USUÁRIO ${who} NO SE ENCUENTRA EN MI BASE DE DATOS*`)
...
type,
count,
timeout: setTimeout(() => (m.reply('*SU TEMPO SE HA TERMINADO*'), delete confirmation[m.sender]), 60 * 1000)
}
}
...
m.reply(`✅ *TRANSFERENCIA HECHA CON SUCESSO:*\n\n*${count} ${type} para* @${(to || '').replace(/@s\.whatsapp\.net/g, '')}`, null, {
mentions: [to]
})
else {
user[type] = previous
_user[type] = _previous
m.reply(`*Erro al transferir ${count} ${type} para* *@${(to || '').replace(/@s\.whatsapp\.net/g, '')}*`, null, {mentions: [to]})
}
clearTimeout(timeout)
delete confirmation[sender]
}
