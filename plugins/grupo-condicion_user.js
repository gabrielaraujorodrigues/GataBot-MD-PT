...
let handler = async (m, { conn, usedPrefix, command, isProprietário, isAdmin, text }) => {
const jsonString = JSON.stringify(global.db.data.chats[m.chat].sCondition)
const parsedData = JSON.parse(jsonString)
  
let phoneNumbers, conditions
if (!(isAdmin || isProprietário)) return m.reply(`*No tienes autorizado usar este comando, solo es para mí Criador(a) y Admins*`)

let txt = `
*Comando Beta*
_Este comando es para definir qué hacer si alguien específico entra al grupo o envía mensagems dentro del mismo._

> Agrega un prefijo de número de teléfono (país), número completo, etiqueta o responde al mensagem del usuário

*Elige qué deseas que pase si ingresa o está en el grupo:*
[1] Remover usuário inmediatamente.
[2] Remover todos sus novos mensagems.
...
- Estas condiciones no aplicarán si el usuário ingresado es mi Criador(a) o Admins.
...
let data = {
grupo: {
usuário: m.quoted && m.quoted.sender ? m.quoted.sender : user.split(', ').map(u => u.trim()),
...
jsonData.grupo.usuário.push(grupoData.grupo.usuário)
...
case "newprefijo":
txt = `*Escribe uno o más prefijos de países que desees que los usuários con dicho prefijo sean removidos del grupo al ingresar*
...
⚠️ *Presta atención*
- Es posible que los prefijos que agregues puedan remover a usuários que coincidan incluso los que ya llevan tempo en el grupo
`.trim()
...
let mensagem = `*Los siguientes prefijos están duplicados:* ${duplicados.map(prefijo => ` \`+${prefijo}\``)}\n\n > *Solo agregué un prefijo*, no es necesario agregar el mismo prefijo varias veces.`
return m.reply(mensagem)
}
