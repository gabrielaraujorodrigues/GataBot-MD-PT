...
if (!fs.existsSync(path)) return await m.reply('⚠️ El arquivo *creds.json* no existe.')

let creds = fs.readFileSync(path)
await conn.reply(m.sender, `📁 *Sessão* (${date})`, fkontak)
...
console.log(`Arquivo .tar.gz creado: ${archive.pointer()} bytes`)
...
m.sender,
{document: fs.readFileSync(tarPath), mimetype: 'application/gzip', fileName: 'GataJadiBot.tar.gz'},
{quoted: m}
)
fs.unlinkSync(tarPath)
})
...
await m.reply(lenguajeGB['smsMalErro3']() + '\n*' + lenguajeGB.smsMensErro1() + '*\n*' + usedPrefix + `${lenguajeGB.lenguaje() == 'es' ? 'reporte' : 'report'}` + '* ' + `${lenguajeGB.smsMensErro2()} ` + usedPrefix + command)
console.log(`❗❗ ${lenguajeGB['smsMensErro2']()} ${usedPrefix + command} ❗❗`)
console.log(e)}}
