...
const tXXxt = `✨ *LINK | URL:* ${shortUrRRl}\n\n${wm}`.trim()
conn.sendFile(m.chat, urRRl.url, 'erro.mp4', tXXxt, m)
await conn.sendMessage(m.chat, {text: waittttt, edit: key})
await new Promise((resolve) => setTimeout(resolve, 10000))
}
} catch {
...
const txt2 = `✨ *LINK | URL:* ${shortUrl2}\n\n${wm}`.trim()
await conn.sendFile(m.chat, resultss, 'erro.mp4', txt2, m)
await conn.sendMessage(m.chat, {text: waittttt, edit: key})
} catch {
try {
const resultssss = await instagramdl(args[0])
const shortUrl3 = await (await fetch(`https://tinyurl.com/api-create.php?url=${args[0]}`)).text()
const txt4 = `✨ *LINK | URL:* ${shortUrl3}\n\n${wm}`.trim()
for (const {url} of resultssss) await conn.sendFile(m.chat, url, 'erro.mp4', txt4, m)
await conn.sendMessage(m.chat, {text: waittttt, edit: key})
} catch {
try {
...
const txt1 = `✨ *LINK | URL:* ${shortUrl1}\n\n${wm}`.trim()
await conn.sendFile(m.chat, videoig, 'erro.mp4', txt1, m)
await conn.sendMessage(m.chat, {text: waittttt, edit: key})
} catch (e) {
