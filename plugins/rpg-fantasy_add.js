...
_Este comando te permite agregar novos personajes a la base de datos._

*Uso:*
\`${usedPrefix + command}\` link + nombre + origen + info + clase + tipo

*Parámetros:*
\`url:\` » Link de la imagen (debe comenzar con 'https://telegra.ph/file/').\n
...
\`info\` » Realiza una breve descrição del personaje.\n
...
> *Nota*
> _Para obtener el link a la imagen puedes usar el coamndo *${usedPrefix}tourl* respondiendo a la imgen, también puedes mejorar la calidad de imagen respondiendo a la imagen *${usedPrefix}hd*_
...
return conn.reply(
m.chat,
'> *¡Por favor, ingresa un link de imagen válido!*\n\n*Ejemplo:*\nhttps://telegra.ph/file/13739fe030f0a5c8cdd9c.jpg',
m
)
}
...
const formattedClass = classInput
.trim()
.toLowerCase()
.split(' ')
...
return conn.reply(
m.chat,
'> *¡Clase no válida!* Sólo se aceptan las siguientes:\n\n`Común, Poco Común, Raro, Épico, Legendario, Sagrado, Supremo, Transcendental`',
m
)
}

const formattedType = typeInput
.split(/[:;/]/)
...
const reply = await conn.reply(
m.chat,
'> *¡Personaje agregado exitosamente!*\n\nResponde a este mensagem con "enviar" o "👍" sólo si deseas enviar los personajes a mis criadores para que lo agreguen en *GataBot*.',
m
)
...
`*Solicitud de @${m.sender.split('@')[0]} Para agregar personajes de Fantasy RPG en GataBot*`,
null,
{mentions: [m.sender]}
)
await conn.sendMessage(
'593968263524@s.whatsapp.net',
{document: databaseFantasyAdd, mimetype: 'application/json', fileName: `fantasyAdd_${m.sender}.json`},
{quoted: m}
)
await conn.reply('593968263524@s.whatsapp.net', `${jsonString}`, m)
await conn.reply(m.chat, '¡Arquivo enviado a mis criadores! Sigue agregando más personajes que quieras que esten en GataBot', m)
}
}
} catch (erro) {
console.erro('Erro al procesar la solicitud: ', erro)
conn.reply(m.chat, '¡Ocurrió un erro al procesar la solicitud!', m)
}
}
